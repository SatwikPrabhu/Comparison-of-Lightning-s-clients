...
def shortest_cost_fun(G,amount,u,v,prob):
    return 1

def least_delay_cost_fun(G,amount,u,v,prob):
    return G.edges[v,u]["Delay"]

def cheapest_cost_fun(G,amount,u,v,prob):
    # if(direct_conn):
    #     return 0
...
def lnd_cost_fun(G, amount, u, v,prob):
    # if direct_conn:
...
def eclair_cost_fun(G, amount, u, v,prob):
    # if direct_conn:
    #     return 0
    fee = G.edges[v,u]['BaseFee'] + amount * G.edges[v,u]['FeeRate']
...
def calc_params(G, path, amt):
    ...
    for i in range(len(path)-2,0,-1):
        ...
        dist += fee * (ndelay * DELAY_RATIO + (1-ncapacity) * CAPACITY_RATIO + nage * AGE_RATIO)
    ...
    while 0!=pq.qsize():
        ...
        for [v,curr] in G.in_edges(curr):
            if v == source and G.edges[v,curr]["Balance"]>=amount[curr]:
                # print(curr)
                # print(amount[curr],"yes",G.edges[v,curr]["Balance"])
                if cost_function == lnd_cost_fun:
                    cost = dist[curr] + amount[curr]*G.edges[v,curr]["Delay"]*LND_RISK_FACTOR
                elif cost_function == eclair_cost_fun or cost_function == cheapest_cost_fun:
                    cost = dist[curr]
                elif cost_function == least_delay_cost_fun:
                    cost = dist[curr] + G.edges[v,curr]["Delay"]
                elif cost_function == shortest_cost_fun:
                    cost = dist[curr] + 1
                else:
            ...
            if(G.edges[v, curr]["Balance"] + G.edges[curr, v]["Balance"] >= amount[curr]) and v not in visited and v!=source:
                cost = dist[curr] + cost_function(G,amount[curr],curr,v,prob[curr])
                if cost < dist[v]:
                    dist[v] = cost
                    paths[v] = [v] + paths[curr]
                    delay[v] = G.edges[v,curr]["Delay"] + delay[curr]
    ...
    for k in range(1, 3):
        ...
        for i in range(len(A)-1, 0,-1):
            ...
            if i == len(A)-1:
                ...
                amt_spur = amt_spur + G.edges[A[i], A[i + 1]]["BaseFee"] + amt_spur * G.edges[A[i], A[i + 1]]["FeeRate"]
            rootpath = A[i:len(A)]
            # print(spurnode,rootpath,len(B))
            for j in range(0, k):
                p = B[j]
            ...
            if flag == 0:
                ...
                paths[leng]["Dist"] = di
                leng += 1
                #print(k,leng,totalpath,spurnode,spurpath,di)
                #print(totalpath,paths)
    ...
    while 0 != pq.qsize():
        ...
        for [v, curr] in G.in_edges(curr):
            ...
            if (G.edges[v, curr]["Balance"] + G.edges[curr, v]["Balance"] >= a) and visited[v]<3 and v not in p:
                cost = di + cost_function(G, a, curr, v,0)
